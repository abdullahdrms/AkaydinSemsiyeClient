...
// material ui
import { Box, Chip, Grid, Stack, Button, Switch, Divider, TextField, InputLabel, Typography, Autocomplete, DialogContent, DialogActions, FormControlLabel, FormControl, FormGroup, Checkbox, RadioGroup, Radio } from '@mui/material';
...
export default function FormOrderUpdate() {
  ...
  const VillaSchema = Yup.object().shape({
    ...
    deliveryAdressId: Yup.object().test("isValid", "Bu alan zorunlu", (value) => {
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        if (parseInt(formik.values.deliveryLocation) === 1) {
          ...
          fd.append('CustomerAddressId', formik?.values?.deliveryAdressId?.id)
        }
        ...
        await UpdateOrder(fd).then((res) => {
          if (res?.errors || res?.statusCode === 400 || res?.statusCode === 500) {
            ...
            navigate(`/orders/detail/${params.id}`)
          }
          setSubmitting(false)
        })
