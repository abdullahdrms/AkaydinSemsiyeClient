...
// assets
import { Add, Edit, Eye, Location, Trash, PasswordCheck, Personalcard } from 'iconsax-react';

// custom
import Loader from 'components/Loader';
import { useNavigate } from 'react-router-dom';
import PasswordUpdateModal from 'sections/employees/PasswordUpdateModal';
import RoleModal from 'sections/employees/RoleModal';
import EmployeesUpdateModal from 'sections/employees/EmployeesUpdateModal';
            ...
            <MainCard content={false}>
                <Stack direction="row" spacing={2} alignItems="center" justifyContent="space-between" sx={{ padding: 3 }}>
                    ...
                    <Stack direction="row" alignItems="center" spacing={2}>
                        <Button variant="contained" startIcon={<Add />} onClick={() => { navigate("/employees/add"); }} size="large">
                            Personel Ekle
                        </Button>
                    </Stack>
                </Stack>
                                ...
                                <TableBody>
                                    {table.getRowModel().rows.map((row) => (
                                        <TableRow
                                            key={row.id}
                                            onClick={() => {
                                                // navigate(`/orders/villas-show/summary/${row.original.id}`)
                                            }}
                                            style={{ cursor: 'pointer' }}
                                        >
...
export default function EmployeesList() {
    ...
    const [employeeUpdateModal, setEmployeeUpdateModal] = useState(false)
    ...
    useEffect(() => {
        setLoading(true)
        GetAllUser().then((res) => { setData(res); setLoading(false); });
        ...
        () => [
            ...
            },
            {
                ...
                cell: ({ row }) => {
                    ...
                    return (
                        <Stack width={130} direction="row" spacing={0}>
                            ...
                            <Tooltip title="Adresler">
                                <IconButton
                                    ...
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        navigate('/employees/addresses/1')
                                    }}
                                >
                                    <Location />
                                </IconButton>
    ...
    let breadcrumbLinks = [{ title: 'Personel Yönetimi' }, { title: 'Personeller', to: `/employees/list` }];
    ...
    return (
        <>
            ...
            <PasswordUpdateModal open={passwordUpdateModal} modalToggler={setPasswordUpdateModal} />
            <RoleModal open={roleModal} modalToggler={setRoleModal} />
            <EmployeesUpdateModal open={employeeUpdateModal} modalToggler={setEmployeeUpdateModal} />
        </>
    );
}