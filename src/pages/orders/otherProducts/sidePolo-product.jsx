...
export default function SidePoloProduct({ update = false }) {
    ...
    const VillaSchema = Yup.object().shape({
        ...
        shapeSizeType: Yup.string().test("isValid", "Bu alan zorunlu", (value) => {
            if (parseInt(formik.values.shape) === 1) {
                if (parseInt(value) === 2 || parseInt(value) === 3 || parseInt(value) === 4) {
                    return true
                } else {
                    return false
                }
    ...
    let breadcrumbLinks = [{ title: 'Sipariş Yönetimi', to: '/orders/list' }, { title: 'Detay', to: `/orders/detail/${data?.orderId}` }, { title: 'Ürün Detayı', to: `/orders/detail/product-detail/${updateOrderId}` }, { title: 'Ürün Düzenle' }, { title: 'Yandan Gövdeli Şemsiye', }]
